class Nagasaki < ApplicationRecord
  require 'mechanize'

  def self.nagasaki_scrape(params)
    agent = Mechanize.new
    page = agent.get('https://www.tvkingdom.jp/chart/57.action?head=&span=24&chartWidth=950&cellHeight=3&sticky=false&descriptive=true&buttonType=0')
    ...
    @data.each do |d| 
      ...
      tv ={}
      @tv = Nagasaki.new
      Nagasaki.create(title: d[:title], time: d[:time], overview: d[:overview], detail: d[:detail], performer: d[:act], prefecture_id: params[:prefecture_id])
